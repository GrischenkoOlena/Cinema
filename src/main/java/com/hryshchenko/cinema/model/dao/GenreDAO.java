...
public class GenreDAO extends AbstractDAO<Integer, Genre>{
    ...
    public List<Genre> findAll() throws DAOException {
        List<Genre> genres;
        try {
            genres = genreQueryBuilder.executeAndReturnList(dbManager, Query.GET_ALL_GENRES, 0,10);
    ...
    public Genre findEntityByKey(Integer id) throws DAOException {
        Genre genre;
        try {
            genre = genreQueryBuilder.executeAndReturnValue(dbManager, Query.GET_GENRE_BY_ID, id);
        } catch (SQLException e){
    ...
    public boolean delete(Genre genre) throws DAOException {
        boolean result;
        try {
            result = genreQueryBuilder.execute(dbManager, Query.DELETE_GENRE, genre.getId());
        } catch (SQLException e){
    ...
    public boolean create(Genre genre) throws DAOException {
        boolean result;
        try {
            result = genreQueryBuilder.execute(dbManager, Query.CREATE_GENRE, genre.getGenre());
        } catch (SQLException e){
    ...
    public boolean update(Genre genre) throws DAOException {
        boolean result;
        try {
            result = genreQueryBuilder.execute(dbManager, Query.UPDATE_GENRE, genre.getGenre(), genre.getId());
        } catch (SQLException e){
